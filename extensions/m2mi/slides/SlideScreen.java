...
// Hidden constructors.
	 ...
	 */
	public SlideScreen(ATReplacementFactory factory)
		{
		super ("Slide Screen");
		myself = this;
				...
				(new AsyncScreenSelectionListener()
					...
					public void theatreSelected
						...
						if (myTheatreHandle != null)
							{
							//((Multihandle) myTheatreHandle).detach(myScreenObject);
							mySlideSet.clear();
							}
						if (theHandle != null)
							{
		...
		try {
			myDiscoverableScreenObject = factory.makeDiscovery(myScreenChooser);
		} catch (Exception e1) {
			System.err.println("ERROR wrapping discovery in AT object: "+e1.getMessage());
			e1.printStackTrace();
		}
		...
		try {
			...
			System.err.println("ERROR wrapping screen in AT object: "+e1.getMessage());
			e1.printStackTrace();
		}
